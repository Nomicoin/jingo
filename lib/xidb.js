...
Xidb.prototype.getLatestSnapshotLink = function() {
...
Xidb.prototype.resolveSnapshotLinks = function(versions) {
  var self = this;

  var resolved = versions.map(function(version) {
    if (version.snapshot) {
      var link = version.snapshot;
      ...
      version.snapshot = metadata.xidb;
    }
    else {
...
Xidb.prototype.getMetaVersions = function(xid) {
  ...
  var versions = files.map(function(file) {
    ...
    var metadata = JSON.parse(data).xidb;
