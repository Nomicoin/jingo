...
Xidb.prototype.getMetalink = function(commitId, name, ignoreCase) {
  var snapshot = this.getSnapshot(commitId);
...
Xidb.prototype.getComments = function(snapshot, xlink) {
  ...
  comments = comments.map(function(clink) {
    ...
    metadata.comment.authorName = metadata.comment.email;
...
Xidb.prototype.addComment = function(email, xlink, comment, cb) {
  var command = "python addComment.py -a " + email + " -x " + xlink;
  var scriptDir = "/home/david/dev/viki/scripts";

  child = exec(command, { cwd: scriptDir }, function(error, stdout, stderr) {
