...
module.exports.initialize = function(config) {
  ...
  function requireRSSAuthentication(req, res, next) {
    if (!res.locals.user && config.get("authentication").rsshttp.enabled) {
      res.redirect("/auth/basic_rss");
    } else {
      // RSS Basic Authentication not valid for site content
      req.logout(); 
      next();
    }
  }
  ...
  if (!app.locals.config.get("authorization").anonRead) {
    ...
    app.all("/rss", requireRSSAuthentication);
  }
  ...
  app.use(require("../routes/wiki"))
     ...
     .use(require("../routes/meta"))
<<<<<<< HEAD
     .use(require("../routes/admin"))
=======
     .use(require("../routes/rss"))
>>>>>>> origin/master
