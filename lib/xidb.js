...
Xidb.prototype.getComments = function(snapshot, xlink) {
  ...
  for(var xid in assets) {
    var asset = assets[xid];
    if (asset.name.search(/evals.*md$/) == 0) {
...
Xidb.prototype.getVotes = function(snapshot, xlink) {
  var assets = snapshot.assets;

  votes = [];
  for(var xid in assets) {
    var asset = assets[xid];
    if (asset.name.search(/evals.*json$/) == 0) {
      console.log(">>>", asset.name);
      votes.push(this.createLink(xid, asset.commit));
    }
  }

  var self = this;
  votes = votes.map(function(vlink) {
    var metadata = self.getMetadataFromLink(vlink);

    if ('vote' in metadata && metadata.vote.ref == xlink) {
      metadata.vote.authorLink = "/meta/" + metadata.vote.author;
      metadata.vote.age = moment(metadata.base.timestamp).fromNow();
      metadata.vote.link = "/meta/" + metadata.base.xlink + "/asset";
  ...
  votes = votes.filter(function(metadata) { return metadata != null; });
  ...
  for(var i = 0; i < votes.length; i++) {
    var meta = votes[i];
    var author = meta.vote.author; // tbd key = author xid
    votesByAgent[author] = meta;
  }

  votes = [];

  for (var agent in votesByAgent) {
    var metadata = votesByAgent[agent];
    
    //this.getBlob(metadata.base.branch, metadata.asset.sha, function(err, content) {
    //  if (content) {
    //    metadata.voteDoc = JSON.parse(content);
    //    console.log("---", metadata);
    //  }
    //});

    var docPath = path.join(this.wikiDir, metadata.asset.name);
    var data = fs.readFileSync(docPath);
    metadata.voteDoc = JSON.parse(data);
...
Xidb.prototype.addVote = function(user, xlink, body, cb) {
  var command = "python addVote.py -a " + user.displayName + " -x " + xlink + " --vote " + body.vote;
