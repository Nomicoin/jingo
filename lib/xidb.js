...
Xidb.prototype.getProjectIndex = function() {
  ...
  for(var repoDir in projects) {
    var cid = this.getHeadCommit(repoDir);
    var xid = this.getProjectId(repoDir, cid);
    var xlink = this.createLink(xid, cid);

    index.push({'name': repoDir, 'xlink':xlink});
  }

  return index;
}

// Xidb.prototype.getBranchHeadLink = function() {
//   var commitId = this.getHeadCommit();
//   var projectId = this.getProjectId(this.repoDir, commitId);
//   var link = this.createLink(projectId, commitId);
...
//   // deprecated by getLatestSnapshotLink?
