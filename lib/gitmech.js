...
var gitMech = {
  ...
  show: function(path, version, callback) {
    ...
    if (path) {
      target = version + ":" + docSubdir + path;
    }

    gitSpawn(["show", target], function(err, data) {
      if (err) {
      //  console.log(err);
        callback(err);
      } else {
  ...
  setGitDir: function(tempDir) {
    var prevDir = gitDir;

    workTree = tempDir.slice(0, tempDir.indexOf('.git'));
    gitCommands = ["--git-dir=" + tempDir, "--work-tree=" + workTree];

    return prevDir;
  },

  getBlob: function(version, callback) {

    gitSpawn(["cat-file", "blob", version], function(err, data) {
      if (err) {
      //  console.log(err);
        callback(err);
      } else {
  ...
  lsRev: function(pattern, rev, callback) {
    gitExec([ "ls-tree", "--name-only", "-r", rev, docSubdir + pattern ], function(err, data) {
      callback(null, data.toString().split("\n").filter(function(v) { return v !== ""; }));
    });
  }
